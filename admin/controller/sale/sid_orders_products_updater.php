<?php
class ControllerSaleSidOrdersProductsUpdater extends Controller { 
	public function index() {  
 		$this->language->load('sale/sid_orders_products_updater');

		$this->document->setTitle($this->language->get('heading_title'));
                
                $this->data['action'] = $this->url->link('sale/sid_orders_products_updater', 'token=' . $this->session->data['token'], 'SSL');
                $this->data['delete'] = $this->url->link('sale/sid_orders_products_updater/removeProducts', 'token=' . $this->session->data['token'] , 'SSL');
			...
			$this->data['success'] = '';
                ...
                if (isset($this->request->post['product_to_delete'])) {
                        ...
                        $this->load->model('sale/sid_orders_products_updater');
                    
                        $product_to_delete_options=$this->model_sale_sid_orders_products_updater->getProductOptions($product_to_delete_id);
                        //$product_options=$this->model_sale_sid_orders_products_updater->getOtpProductOptions($product_to_delete);
                } else {
                        $product_to_delete = "";
                ...
                if (isset($this->request->post['product_to_add'])) {
                        ...
                        $product_to_add_options=$this->model_sale_sid_orders_products_updater->getProductOptions($product_to_add_id);
                } else {
                        $product_to_add = "";
                        $product_to_add_id="";
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
                    $this->load->model('sale/sid_orders_products_updater');
                    //$order_total = $this->model_sale_sid_orders_products_updater->getTotalOrders($data);
                    $data = array(
                    ...
                    $results = $this->model_sale_sid_orders_products_updater->getOrders($data);
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('sale/sid_orders_products_updater', 'token=' . $this->session->data['token'] . $url, 'SSL'),
			'separator' => ' :: '
		);
		...
		$this->template = 'sale/sid_orders_products_updater.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
        ...
        public function options(){
            ...
            if (isset($this->request->get['product_id'])){
                    ...
                    $product_otp=$this->model_sale_sid_orders_products_updater->getOtpProductOptions($product_id);
        ...
        public function removeProducts(){
            if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
                    ...
                    //////$this->log->write($this->request->post['product_to_add_id']);
                    if (isset($this->request->post['product_to_add'])) {
                            $product_to_add= $this->request->post['product_to_add'];
                            $product_to_add_id=$this->request->post['product_to_add_id'];
                    } else {
                    ...
                    if ($product_to_delete_id == "" || $product_to_add_id== ""){
                        $this->session->data['error_warning']=$this->language->get('error_required');
                    } else{
