...
class ControllerModuleFacebooklogin extends Controller {
	public function index() { 
		...
		$this->document->addStyle('view/javascript/facebooklogin/font-awesome/css/font-awesome.min.css');
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!empty($_POST['OaXRyb1BhY2sgLSBDb21'])) {
				$this->request->post['FacebookLogin']['LicensedOn'] = $_POST['OaXRyb1BhY2sgLSBDb21'];
			}
			if (!empty($_POST['cHRpbWl6YXRpb24ef4fe'])) {
				$this->request->post['FacebookLogin']['License'] = json_decode(base64_decode($_POST['cHRpbWl6YXRpb24ef4fe']),true);
			}
		...
		$this->data['moduleData'] = (isset($this->data['data']['FacebookLogin']))?$this->data['data']['FacebookLogin'] :array();
		...
		foreach ($this->data['stores'] as $store) {
			...
			$is_https = $this->model_module_facebooklogin->is_https($login_url);
			
			if ($is_https) {
				$redirect_url = $store['ssl'] . 'index.php?route=account/facebooklogin';
			} else {
				$redirect_url = $store['url'] . 'index.php?route=account/facebooklogin';
			}
