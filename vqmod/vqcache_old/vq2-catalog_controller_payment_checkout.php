...
class ControllerPaymentCheckout extends Controller {
	protected function index() {
		...
		if ($order_info) {
			...
			$reject = $this->url->link('payment/checkout/reject', '', 'SSL'); // Hylätty-linkki
			$delayed = $this->url->link('payment/checkout/callback', '', 'SSL'); // Viivästytetty-linkki
			$country = 'FIN'; // Maa
			$currency = 'EUR'; // Valuutta
			$device = $this->config->get('checkout_device'); // Maksupääte: 1=html, 10=xml
			$content = $this->config->get('checkout_content'); // Maksun tyyppi
			$type = '0'; // Maksutavat
			$algorithm = '2'; // Algoritmi
			$delivery_date = date('Ymd'); // Toimituspäivä
			...
			if($device == 1 || $curlError) {
				...
				$this->data['amount'] = $amount; // Maksun määrä -> Euro sentteinä
	...
	public function callback() {
		...
		$algorithm = $this->request->get['ALGORITHM']; // Käytetty algoritmi
		...
		if($return_mac === $mac) {
			if($this->config->get('checkout_debug')) {
				...
				$geturl = $this->request->get;
				$getdata = print_r($this->request->get, true);
				file_put_contents(DIR_LOGS . "checkout.txt", "{$aika} {$viesti}\n{$geturl}\n{$getdata}\n", FILE_APPEND);
			}

			$this->load->model('checkout/order');
			$order_info = $this->model_checkout_order->getOrder($stamp);

			if($status == 2 || $status == 8) {
				$comment = sprintf($this->language->get('text_reference'), $reference);
				$this->model_checkout_order->confirm($stamp, $this->config->get('checkout_ok_status_id'), $comment, true);
				...
				$comment = sprintf($this->language->get('text_reference'), $reference);
				$this->model_checkout_order->confirm($stamp, $this->config->get('checkout_delayed_status_id'), $comment, true);
				if($this->config->get('checkout_log')) {
					$aika = date("Y-m-d H:i:s");
					$tilaus = $this->language->get('text_order_number') . $this->request->get['STAMP'];
					$viesti = sprintf($this->language->get('text_checkout_status'), $this->language->get('text_status_' . $status), $status);
				...
				$comment = sprintf($this->language->get('text_reference'), $reference);
				$this->model_checkout_order->confirm($stamp, $this->config->get('checkout_unknown_status_id'), $comment, true);
				if($this->config->get('checkout_log')) {
					$aika = date("Y-m-d H:i:s");
					$tilaus = $this->language->get('text_order_number') . $this->request->get['STAMP'];
					$viesti = sprintf($this->language->get('text_checkout_status'), $this->language->get('text_status_' . $status), $status);
