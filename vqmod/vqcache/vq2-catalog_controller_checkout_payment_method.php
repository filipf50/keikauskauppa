  	...
  	public function index() {
		...
		if (!empty($payment_address)) {
			...
			foreach ($results as $result) {
				...
				if ($this->config->get($result['code'] . '_status') && $result['code'] != "layaway") {
					...
					if (!$this->customer->isLogged() || in_array($this->customer->getCustomerGroupId(), $customer_groups_allowed)) {
						...
						if (!empty($store_allowed)) {
							foreach ($store_allowed as $store) {
								if ($store == $this_store) {
									if ($this->config->get('layaway_status') && $this->config->get('layaway_allow_admin') && !isset($this->session->data['layaway_payment'])) {
