function nwaListEvent(e, t, n) {
    if (n == 1 || n == 2) {
        $("#nwa_list_product_id").val(t);
        $("#nwa_list_msg").hide();
        var r = e.pageY + 15;
        $("#nwa_list").css("top", r + "px");
        var i = e.pageX + $("#nwa_list").width() > $(window).width() + 15 ? $(window).width() - ($("#nwa_list").width() + 15) : e.pageX - 30;
        $("#nwa_list").css("left", i + "px");
...
$.fn.nwaList = function(e) {
    return this.each(function() {
        ...
        if ($.inArray(t, e.out_stock_list) != -1) {
            $(this).before('<span class="nwa_list_title">' + e.title + "</span><br/>");
            if (e.gray_style) {
            ...
            $(this).attr("onclick", "nwa_list(" + e.show_mode + ");");
            $(this).attr("onmouseenter", "nwa_list(" + e.show_mode + ");");
            if (e.show_mode == 1) {
                $(this).attr("onclick", "nwaListEvent(event," + t + "," + e.show_mode + ");");
                $(this).attr("onmouseenter", "nwaListEvent(event," + t + "," + e.show_mode + ");")
            } else {
                $(this).attr("onclick", "nwaListEvent(event," + t + "," + e.show_mode + ");")
            }
        }
...
$(function() {
    ...
    $("#nwa_list_register,#nwa_list_register span").live("click", function(e) {
        $.ajax({
            ...
            success: function(e) {
                if (e["sucess"]) {
                    $("#nwa_list_msg").html('<div id="nwa_list_success">' + e["msg"] + "</div>")
                } else {
                    $("#nwa_list_msg").html('<div id="nwa_list_error">' + e["msg"] + "</div>")
                }
                $("#nwa_list_msg").show()
            }
        })
    ...
    $("#nwa_product_register").live("click", function() {
        $.ajax({
            ...
            success: function(e) {
                if (e["sucess"]) {
                    $("#nwa_product_msg").html('<div id="nwa_product_success">' + e["msg"] + "</div>")
                } else {
                    $("#nwa_product_msg").html('<div id="nwa_product_error">' + e["msg"] + "</div>")
                }
...
$.fn.nwaOption = function(e, t, n, r) {
    ...
    if (typeof nwa_options[e] != "undefined" && typeof nwa_options[e][t] != "undefined") {
        ...
        $("#nwa_product_box_title").text(n + " : " + r + " " + nwa_title);
        if ($(this).is("input")) {
...
$.fn.nwaProduct = function(e) {
    ...
    if (e.show_mode == 0) {
        ...
        if (e.gray_style) {
            $(t).addClass("nwa_product_button_custom");
            if ($(t).is("input")) {
                $(t).val(e.text)
            } else {
        ...
        $(t).parent().html($(t));
        $(t).before('<b id="nwa_product_box_title">' + e.title + "</b><br/>");
        $(t).unbind();
        $(t).click(function(e) {
            $.colorbox({
                overlayClose: true,
